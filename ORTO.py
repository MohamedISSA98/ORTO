...
class ORTO(HoeffdingTree, base.Regressor):
    ...
    def _attempt_to_split(
        ...
        if len(best_split_suggestions) < 2:
            ...
            if best_suggestion.merit > 0.0 and (
                ...
                should_split = True
            add_options = False
            if best_suggestion.merit > 0.0 and should_split==False:
                s = []
                for i in range(2,len(best_split_suggestions)+1):
                    ratio = (best_split_suggestions[-i].merit / best_suggestion.merit)> 1 - hoeffding_bound
                    if ratio:
                        s.append(-i)
                        add_options = True
        ...
        if add_options:
            for i in s:
                split_decision = best_split_suggestions[i]
                if split_decision.feature is None:
                    # Pre-pruning - null wins
                    leaf.deactivate()
                    self._n_inactive_leaves += 1
